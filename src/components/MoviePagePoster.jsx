...
const MoviePagePoster = ({ movie }) => {
  ...
  const onReview = async (movie, review) => {
    if (auth.currentUser === null) {
      ...
      await checkMovieReviewedDB(movie).then(async () => {
        if (movieReviewed) {
          console.log(
            "movie is already reviewed, maybe setup an edit/alert u cant edit ur review mmm"
          );
          ...
          console.log(
            "create movie db/check if it exists + send review there too"
          );
        }
        //i can replace with a ternary operator?
      });
  ...
  const checkMovieInDB = async (movie) => {
    ...
    console.log(
      "checked for movie in my db collection, result was: ",
      movieExists
    );
  };
  ...
  return (
    <div key={movie.id}>
      ...
      <div className="review-form">
        ...
        <button onClick={() => onDeleteReview(movie, review)}>
          Delete Review
        </button>
      </div>
    </div>
